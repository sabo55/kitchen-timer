...
/* ========== 内蔵音 & ラジオ用オプション生成（TimerSettingsModal からの行数削減） ========== */
...
// TimerSettingsModal から呼ぶ想定：
// buildRadioSoundList({ withBuiltins:true, withTimes:false, withCustom:true, withSilent:true, exclude:[...] })
// これで置き換え
export function buildRadioSoundList(
  opts: {
  ...
  const {
    withBuiltins = true,
    withTimes = false,   // 設定画面ではデフォルト混ぜない
    withCustom = true,
    withSilent = true,
    exclude = [],
  } = opts;
  ...
  // 時間アナウンス（内蔵 → レジストリ有効分 → 事前登録23個）
  if (withTimes) {
    try { list.push(...TIME_ITEMS); } catch {}
    try { list.push(...buildTimeAnnounceOptions()); } catch {}
    try { list.push(...buildAllTimePresetOptions()); } catch {}
  }
...
export type SoundGroup = { id: string; label: string; items: SoundOption[] };
export function buildRadioSoundGroups(opts?: {
  ...
  // 無音は単独扱いにすると選びやすい（先頭固定）
  const silent = buildRadioSoundList({ ...opts, withBuiltins: false, withTimes: false, withCustom: false, withSilent: true, exclude: ex });
  if (silent.length) groups.push({ id: "silent", label: "無音", items: silent });

  const basic = uniqById(buildRadioSoundList({ ...opts, withBuiltins: true, withTimes: false, withCustom: false, withSilent: false, exclude: ex }));
  if (basic.length) groups.push({ id: "basic", label: "基本", items: basic });

  const times = uniqById(buildRadioSoundList({ ...opts, withBuiltins: false, withTimes: true, withCustom: false, withSilent: false, exclude: ex }));
  if (times.length) groups.push({ id: "time", label: "時間アナウンス", items: times });

  const custom = uniqById(buildRadioSoundList({ ...opts, withBuiltins: false, withTimes: false, withCustom: true, withSilent: false, exclude: ex }));
  if (custom.length) groups.push({ id: "custom", label: "登録音", items: custom });
